...
export class PdfAction implements CommandAction {
    ...
    public async handle(options: CommandOptions): Promise<void> {
        const { files, outputFileName } = await this.promptOptions.handleMissingOptions(options, FileExtension.PDF);

        console.log("files", files, "outputFileName", outputFileName);
    }
}
